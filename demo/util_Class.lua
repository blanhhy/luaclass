...
-- Luaclass 提供了一个 Class 工具类, 用于模拟传统的 OOP 形式
...
-- 也由于 Class 不接受定义体, 你不能在标准创建器中指定 metaclass = Class; 即使它确实是一个元类
-- 如果真的那样做了, 定义体被会忽略, 和直接实例化 Class 的效果没有不同
...
-- 此外, luaclass 还提供了 new 作为实例化方法
-- 使用 clazz:new() 和 clazz() 是等价的
local stu2 = Student:new("Joe", 9)
super(stu2):sayHello()

-- 有很多传统的类库没有类名一说, 就靠一个变量名
-- luaclass 也模拟了这种用法, 可以匿名创建, 实际类名将会是一个随机字符串
-- 命名空间则是 lua.class.anonymous, 此命名空间为kv弱表, 不会延长匿名类的生命周期

-- 标准创建器创建匿名类, name为空串或缺省即可
-- local clazz = class () {...}

-- Class 匿名创建类, 不带字符串参数 (可以有基类) 即可
local Animal = Class()
...
print(Animal) -- 输出类似 Class_xxxxxxxxxx
